        ...
        handleNodeWithTracers runP = do
          ...
          Async.withAsync (handlePeersListSimple trace nodeKernelData)
              $ \_peerLogingThread ->
                ...
                (if ncEnableP2P nc
                  then handleSimpleNode p runP trace tracers nc (setNodeKernel nodeKernelData)
                  else handleSimpleNodeNonP2P p runP trace tracers nc (setNodeKernel nodeKernelData))
                `finally`
